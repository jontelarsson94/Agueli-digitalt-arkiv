<?php
  ...
  $year_tags = $database->select("tags", [
    "id"
    ], [
      "AND" =>[
      "name[>=]" => $min,
  ...
  $year_tag_ids = array();
  foreach($year_tags as $year){
    array_push($year_tag_ids, $year["id"]);
  }

  $year_articles = $database->select("article_tags", [
    "article_id"
    ], [
    "tag_id" => $year_tag_ids
    ]);

  $year_article_ids = array();

  foreach($year_articles as $id){
    array_push($year_article_ids, $id["article_id"]);
  }
